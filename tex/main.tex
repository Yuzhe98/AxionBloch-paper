...
\affiliation{Helmholtz Institute Mainz, 55128 Mainz, Germany}
...
In the presence of magnetic fields or axion-induced pseudomagnetic fields, the magnetization $\mathbf{M}$ evolves as
\begin{align}
...
\end{align}
where $\gamma$ is the gyromagnetic ratio, $\mathbf{B}_{\mathrm{eff}}$ is the total effective field, $T_1$ and $T_2$ are relaxation times, and the subscripts $x$, $y$, $z$ denote the Cartesian components. 
...
Given a time step $\Delta t$, an effective magnetic field, and initial magnetization $\mathbf{M}^0$, the magnetization at the $(n+1)$-th step can be found via 
...
A \SI{1}{\%}-polarization ethanol sample is used as an example demonstrating the evolution of polarization due to the $T_1$ relaxation. 
...
\begin{table*}
    ...
    $\chi^2$ indicating the discrepancy is defined in \cref{eq:chisq}; $\chi^2\ll 1$ indicates a well-calibrated simulation.
    Here $\nu_\mathrm{L}$ stands for Larmor frequency, determined by the bias magnetic field. 
    $\nu_\mathrm{RCF}$ is RCF frequency. 
    $\nu_\mathrm{L}$ and $\nu_\mathrm{RCF}$ are varied from \SI{1}{\kHz} to \SI{1}{\GHz}. The bias field inhomogeneity is swept from \SI{.1}{\ppm} to \SI{20}{\ppm}. 
    $T_1$ ranges from \SI{1}{\ms} to \SI{1}{\kilo\second}. 
    The integration step size is chosen to be $T_2^* / 500$. 
    % $\chi^2$ can be even smaller by using a smaller step size at the cost of runtime consumption. 
    Finer step sizes yield lower $\chi^2$ at increased computational cost. 
    More technical details of the calibrations can be found in the test suite. 
    }
...
The author acknowledges helpful discussions with Hendrik Bekker, Dmitry Budker, and Alexander Sushkov, and thanks the CASPEr Collaboration for its support. 
