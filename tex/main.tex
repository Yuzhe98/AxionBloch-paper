...
\begin{figure*}[ht]
    \centering
    \fbox{\includegraphics[scale=0.95]{figures/diagram_v1.1.drawio.pdf}}
...
A quantitive analysis is included by the end of the section. 
...
% We start with examples of common NMR experiments 
...
\end{itemize}
The behaviors described above can be found in the calibration example illustrated in \cref{fig:calibration_T2_T2star}\,(a), indicating a rational simulation. 
...
We use a \SI{1}{\%}-polarization ethanol sample as an example demonstrating the evolution of hyperpolarization due to the $T_1$ relaxation. 
...
As can be seen in \cref{fig:calibration_T1_HP}, the $T_1$ relaxation followed the expectation. 
More of such simulation can be applied in calibrating $T_1$ relaxation. 
...
Accuracy of simulations is evaluated by the discrepency between theoretically-expected magnetization and simulation results, characterized by 
\begin{equation}
...
$\sum$ stands for the summary over integration steps. 
Well-calibrated simulation should give $\chi^2\ll 1$. 
...
The discrepencies are in general small ($\chi^2 < 10^{-4}$). 
If a certain precision of simulation is demanded, $\chi^2$ value can a reference for consideration. 
\begin{table*}
...
% It also allows one to verify the correctness and stability of the numerical integration. 
% Examples \cref{sec:examples} can help calibrate simulations qualitatively. 
% However, to ensure accurate results, quantitative calibrations are necessary, wchich are presented as follows. 
