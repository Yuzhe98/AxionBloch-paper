...
Ultralight bosonic dark matter with normal-matter spins can be described as the action of a pseudomagnetic field. We model such interaction along with the usual magnetic interactions using spin-evolution (Bloch) equations. 
% Hypothetic beyond-standard-model interactions between axions and fermionic spins might induce magnetic signals. 
We present \axionbloch{}, an open-source Python package for simulating spin dynamics induced by both usual and exotic interactions. 
...
The signal signatures are crucial for designing experimental searches and data analysis. 
...
In this paper, we refer to axions and ALPs as ``axions'' for simplicity. \YZ{think about how to make it transparent. }
...
There have been dedicated studies of the axion-induced spin dynamics\,\cite{Dror2023_SensitivityAxion, Yuzhe2023_FrequencyScanning, Aybas_2021_Quantum_sensitivity}, providing axion signal signatures and experimental sensitivity to axion fields. 
...
We adopt the International System of Units (SI) for inputs and outputs in the package. All physical inputs, internal computations, and outputs are consistently expressed in SI base and derived units. 
...
% \vspace{0.1in}
\caption{Examples of operations of physical quantities using \texttt{axionbloch.enphylope} and \texttt{axionbloch.enphylope}.\YZ{Gauss->gauss}}
...
This class supports arithmetic operations on the quantities while preserving unit consistency. 
...
In \texttt{axionbloch.constants}, we defined not only fundamental physical constants such as speed of light and the Planck constant, but also commonly-used constants in axion and NMR research, like gyromagnetic ratios. 
...
\end{itemize}
We can observe the behaviors above in the calibration example illustrated in \cref{fig:calibration_T2_T2star}\,(a). 
% \YZ{should we add quantitative analysis of the calibration?}
...
\label{sec:CW_NMR}
In the continuous-wave (CW) NMR calibration, an oscillating magnetic field is applied. When the magnetic field strength $B$ is weak ($\gamma B T_2^*\ll 1$) and on resonance, the transverse magnetization grows with $\gamma B T_2^* (1 - e^{-t/T_2^*})$\,\cite{Abragam1961_NuclearMagnetism}.
