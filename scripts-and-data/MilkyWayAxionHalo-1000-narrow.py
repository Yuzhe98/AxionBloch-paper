...
#     pol=sample.pol,
# )
MW_data = np.load("figures/Axion-Xe_NMR-simulations-1000.npz")
...
for i, ax in enumerate([MW_Bx_ax, MW_By_ax]):
    ax.fill_between(
        MW_timeStamp_s,
        (MW_B_vec_mean[:, i] - MW_B_vec_std[:, i]) * B_scale,
        (MW_B_vec_mean[:, i] + MW_B_vec_std[:, i]) * B_scale,
...
MW_M_ax.fill_between(
    MW_timeStamp_s[M_start:],
    (MW_Mxy_mean[M_start:] - MW_Mxy_std[M_start:]),
    (MW_Mxy_mean[M_start:] + MW_Mxy_std[M_start:]),
    color=high_contrast_extended[-2],
    alpha=err_alpha,
)
...
plt.savefig("figures/MilkyWayAxionHalo-1000-narrow.pdf")
plt.show()
